...
class Server:
    def __init__(self):
        ...
        self.model_average_inference_time={'alexnet':120,
                                           'resnet18':120}
    ...
    def compute_average(self,new_time):
        #TODO:compute average
        pass

    def assign_inference_work(self,inference_model):
        ratio = self.model_average_inference_time["alexnet"] / self.model_average_inference_time["resnet18"]
        num_assigned_to_model=round(ratio*self.get_num_worker())
        if not inference_model=="alexnet":
            num_assigned_to_model=self.get_num_worker()-num_assigned_to_model
    ...
    def receiver_inference_program(self):
        ...
        while True:
            try:
                if self.master == HOST:
                    self.assign_inference_work(inference_model)#will change self.worker_set

                    for worker_id in self.worker_set:
                        work_start_time=time.time()
                        self.send_inference_command(worker_id,start_point, start_point + batch_size,inference_model)
                        work_end_time = time.time()
                        self.model_average_inference_time[inference_model]=self.compute_average(work_end_time-work_start_time)#+()/()*self.model_average_inference_time[inference_model]
                    # the format: inference, sender_host, start_point, end_point, inference_model(alexnet or resnet)
                    # according to (batch siez = end_point-start_point) and inferencemodel decides how many workers need to do inference
                else:
                    # call inference and send result back to master and client
                    pass
    ...
    def send_inference_command(self,sendto_hostname,start_point, end_point, inferencemodel="no"):
        '''
        ...
        if self.MembershipList[HOST][1] == utils.Status.LEAVE or HOST not in self.MembershipList:
            ...
            time.sleep(1)
            s.send(f"{utils.Type.INFERENCE}{SE}{HOST}{SE}{start_point}{SE}{end_point}{SE}{inferencemodel}".encode())
        s.close()
    ...
    def inference(self, batch_size, inference_model):

        for start_point in range(0, 10000, batch_size):
            time.sleep(10)
            self.send_inference_command(self.master, start_point, start_point + batch_size,
    ...
    def shell(self):
        print("Welcome to the interactive shell for CS425 MP2. You may press 1/2/3/4 for below functionalities.\n"
              ...
              "13 inference, batch_size, inferen_model name(alexnet or resnet)"
              )
        ...
        while True:
            ...
            if input_str == "1":
                ...
                if len(splited_input_str) == 3 > 0:
                    self.inference(splited_input_str[1], splited_input_str[2])
                else:
                    print("Error: missing or too many " + splited_input_str[0] + " parameter .")
            else:
