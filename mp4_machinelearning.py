...
SLIDING_WINDOW_SECONDS = 10  # measure seconds of query rate
SLIDING_WINDOW_FACTOR=3  #enlarge the window and query rate will divide this factor
...
class Server:
    def __init__(self):
        ...
        self.defined_master = "fa22-cs425-0101.cs.illinois.edu"
        ...
        self.model_average_inference_time = {'alexnet': 100, 'resnet18': 100}
        self.model_inference_time_list = collections.defaultdict(list)  # {"alexnet": [], "resnet18": []}
        self.worker_set = collections.defaultdict(
            list)  # key=(inference_model,query number),value=list of (assigned vm,start_point,end_point,working/finished,start_time,end_time) 'w'means working , 'f' means finished
        self.inference_result_list = collections.defaultdict(list)  # key:(inference_model,query_number),  value: result
        self.working_vm_set = collections.defaultdict(
            list)  # key=(vm_hostname),value=list of (inference_model,querynumber,start_point,end_point)
        self.query_rate = {"alexnet": [0, 0, 0, 0], "resnet18": [0, 0, 0, 0]}  # c1 : [2,5,275,500]
        self.query_rate_slidingwindow = {"alexnet": [], "resnet18": []}  # c1:[] list of query time
        self.query_processing_time_meta = {"alexnet": [0, 0, 0, 0,0],
                                           "resnet18": [0, 0, 0, 0,0]}  # c2:[average,q1,q2,q3,sd]
        self.query_processing_time_meta_slidingwindow = {"alexnet": [],
                                                         "resnet18": []}  # c2:[] list of tiple(finished_time,query_processing_time)
        self.processing_time_list = {"alexnet": [], "resnet18": []}  # c2:[] list of processing time
        self.resnet18_query_rate = 0
        self.alexnet_query_rate = 0
        self.resnet18_query_batchsize_rate = 0
        self.alexnet_query_batchsize_rate = 0
        self.resnet18_query_rate = 0
        self.resnet18_query_time = []
        self.alexnet_query_time = []
        self.resnet18 = 0
        self.alexnet = 0


    def join(self, toPrint=True, toRunAllTime=False, sleepTime=1.5):
        '''
    ...
    def assign_inference_work(self, inference_model, querynumber, start_point, end_point):
        ...
        if not self.model_average_inference_time["resnet18"] == 0:
            ratio = self.model_average_inference_time["alexnet"] / self.model_average_inference_time["resnet18"]
        else:  
            ratio = 1.0
        self.get_num_worker()
        num_worker_assigned_to_model = max(min(round(ratio / (ratio + 1) * RATE_FACTOR), self.num_worker),
                                           0)  # 0<=num_assigned<=num_worker #originally RATE_FACTOR=self.num_worker
        if not inference_model == "alexnet":
            ratio_reciprocal = 1 / ratio
            num_worker_assigned_to_model = max(
                min(round(ratio_reciprocal / (ratio_reciprocal + 1) * RATE_FACTOR), self.num_worker), 0)
        # we know num_assigned_to_model, we want to exactly which n worker to assigns: e.g:vm1,vm3,vm5...
        ...
        for i, worker_id in enumerate(assigned_worker_list):
            ...
            self.worker_set[(inference_model, querynumber)].append(
                (worker_id, now_start_point, now_end_point, 'w', time_assigned, time_assigned))
            if (inference_model, querynumber, now_start_point, now_end_point) not in self.working_vm_set[
                worker_id]:  # MODIFYED-XPC
                self.working_vm_set[worker_id].append((inference_model, querynumber, now_start_point, now_end_point))
    ...
    def receiver_inference_program(self):
        ...
        if host is cordinator: put the file to sdfs, choose replica vm, and assign
        '''
        ...
        while True:
            try:
                ...
                if self.master == HOST:  # if (self.master == HOST or self.standbymaster == HOST) and to_assign_hostname==HOST:
                    ...
                    for (
                            worker_id, assigned_start_point, assigned_end_point, working_or_finished, start_time,
                            end_time) in \
                            self.worker_set[
                                (inference_model, querynumber)]:
                        self.send_inference_work(sender_host, sdfs_file, worker_id, assigned_start_point,
                                                 assigned_end_point, working_or_finished, inference_model, querynumber)
                    ...
                    '''self.update_metadata_when_get_inference_result(utils.Type.RESULT, sender_host, sdfs_file,
                                                                   str(start_point), str(end_point),
                                                                   inference_model, str(querynumber),
                                                                   inference_result)#MODIFYED-XPC 12-4'''
    ...
    def list_to_average_q1_q2_q3_stddev(self, mylist):

        return [mean(mylist), np.percentile(mylist, 25), np.percentile(mylist, 50), np.percentile(mylist, 75),
                np.std(mylist)]

    def update_metadata_when_get_inference_result(self, request_type, sender_host, sdfs_file, start_point, end_point,
                                                  inference_model, querynumber, inference_result):
        if DEBUG_MODE:
            print("-" * 80)
            print("We receive %s %s,start%s,end%s:" % (inference_model, querynumber, start_point, end_point))
    
        for (vm_hostname_i, start_point_i, end_point_i, working_or_finished, start_time_i, end_time_i) in \
        self.worker_set[(inference_model, querynumber)]:
            if str(start_point_i) == str(start_point) and str(end_point_i) == str(end_point):
                # we update the finished time
                finished_time = time.time()
                if working_or_finished == 'w' and DEBUG_MODE:
                    print("We want to mark (%s %s %s %s) as finished" % (
                        vm_hostname_i, start_point_i, end_point_i, working_or_finished))
                finished_vm_hostname = vm_hostname_i
                self.worker_set[(inference_model, querynumber)].remove(
                    (vm_hostname_i, start_point_i, end_point_i, working_or_finished, start_time_i, end_time_i))
                # TODO: whether append a (,,,'f') to self.worker_set[(inference_model, querynumber)]
                self.worker_set[(inference_model, querynumber)].append(
                    (vm_hostname_i, start_point_i, end_point_i, 'f', start_time_i, finished_time))
                self.working_vm_set[finished_vm_hostname].remove(
                    (inference_model, querynumber, start_point_i, end_point_i))
                # update query finish number and query rate
                if inference_model == "resnet18":
                    self.resnet18 += int(end_point_i) - int(start_point_i)
                    self.resnet18_query_time.append([time.time(), int(end_point_i) - int(start_point_i)])

                else:
                    self.alexnet += int(end_point_i) - int(start_point_i)
                    self.alexnet_query_time.append([time.time(), int(end_point_i) - int(start_point_i)])

                # self.update_query_meta()
                time_now = time.time()
                if abs(finished_time - time_now) <= SLIDING_WINDOW_SECONDS*SLIDING_WINDOW_FACTOR:
                    # average processed time
                    batch_size_for_model = RESNET_BATCHSIZE if inference_model == "resnet18" else ALEXNET_BATCHSIZE
                    one_work_num = end_point_i - start_point_i
                    average_processing_time = (finished_time - start_time_i) / one_work_num * batch_size_for_model
                    self.query_processing_time_meta_slidingwindow[inference_model].append(
                        (finished_time, average_processing_time))
                for (finished_time_i, processing_time_i) in self.query_processing_time_meta_slidingwindow[
                    inference_model]:
                    if abs(finished_time_i - time_now) > SLIDING_WINDOW_SECONDS*SLIDING_WINDOW_FACTOR:
                        self.query_processing_time_meta_slidingwindow[inference_model].remove(
                            (finished_time_i, processing_time_i))
                self.processing_time_list[inference_model].clear()
                for (finished_time_i, processing_time_i) in self.query_processing_time_meta_slidingwindow[
                    inference_model]:
                    self.processing_time_list[inference_model].append(processing_time_i)
                if not len(self.processing_time_list[inference_model])==0:
                    self.query_processing_time_meta[inference_model] = self.list_to_average_q1_q2_q3_stddev(
                    self.processing_time_list[inference_model])

                #self.model_average_inference_time[inference_model] = self.query_processing_time_meta[inference_model][0]
                break
    ...
    def receiver_inference_result_program(self):
        ...
        while True:
            try:
                ...
                data = conn_socket.recv(BUFFER_SIZE)
                connect = data.decode()
                request_type, sender_host, sdfs_file, start_point, end_point, inference_model, querynumber, inference_result = connect.split(
                    SE)
                self.inference_result_list[str(inference_model) + " " + str(querynumber)].append(inference_result)
                self.update_metadata_when_get_inference_result(request_type, sender_host, sdfs_file, start_point,
                                                               end_point, inference_model, querynumber,
                                                               inference_result)

            except Exception as e:
                pass

    def transfer_failed_inference_work(self, hostname):  # hostname is failed vm hostname #MODIFYED-XPC
        if DEBUG_MODE:
            print("We detect  %s failed,we are transfering work to others" % hostname)
        count_print = 0
        for (inference_model, querynumber, start_point, end_point) in self.working_vm_set[hostname]:
            for (vm_hostname_i, start_point_i, end_point_i, working_or_finished, start_time_i, end_time_i) in \
                    self.worker_set[(inference_model, querynumber)]:
                if str(start_point_i) == str(start_point) and str(end_point_i) == str(end_point):
                    self.worker_set[(inference_model, querynumber)].remove(
                        (vm_hostname_i, start_point_i, end_point_i, working_or_finished, start_time_i, end_time_i))
                    break
            for replica_host in utils.get_replica_neighbors(hostname):  # TODO:CHECK THIS LOGIC
                if replica_host in self.worker_set[(inference_model, querynumber)] \
                        or self.MembershipList[replica_host][1] == utils.Status.LEAVE \
                        or replica_host == self.master:  # MODIFYED-XPC not to assign replica inference work to master,(we can, so TODO:try if assign to master )
                    continue
                # TODO: to measure the inference time
                '''
                work_start_time = time.time()
                print(worker_id)
                s_worker = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s_worker.connect((utils.ip_dict[USERNAME][worker_id],
                                  INFERENCE_PORT))  # original s.connect((worker_id, INFERENCE_PORT))
                s_worker.send(
                ...
                ss = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                if count_print == 0 and DEBUG_MODE:
                    print("We transfer work to")
                    print(replica_host)
                    count_print += 1
                try:
                    ss.connect((replica_host, INFERENCE_PORT))
                    # ss.send(f"{utils.Type.PUT}{SE}{HOST}{SE}{file}{SE}{file}{SE}{1}".encode())
                    ss.send(
                        f"{utils.Type.INFERENCE}{SE}{HOST}{SE}{inference_model}{SE}{start_point}{SE}{end_point}{SE}{inference_model}{SE}{querynumber}".encode())
                    # self.sdfs_file_process[file].append(replica_host)
                    ss.close()
                    time_now = time.time()
                    self.worker_set[(inference_model, querynumber)].append(
                        (replica_host, start_point, end_point, 'w', time_now, time_now))

                    # self.sdfs_store_dict[replica_host].append(file)
                    self.working_vm_set[replica_host].append((inference_model, querynumber, start_point, end_point))
                except socket.error as exc:
                    continue
                # time.sleep(1)
                # sdfs_send_file_client(file, file, ss)
                break
        self.working_vm_set.pop(hostname, None)

    def send_inference_work(self, sender_host, sdfs_file, worker_id, assigned_start_point, assigned_end_point,
                            working_or_finished, inference_model, querynumber):
        if worker_id == HOST:  # if assigned work to master itself
            inference_result, inference_time = alexnet_resnet.deeplearning(sdfs_file, inference_model,
                                                                           assigned_start_point,
                                                                           assigned_end_point)
            ...
            try:
                ...
                print(e)
            '''return_list = ["172.22.156.2", "172.22.158.2", "172.22.94.2", "172.22.156.3",
                           "172.22.158.3",
                           "172.22.94.3", "172.22.156.4", "172.22.158.4", "172.22.94.4", "172.22.156.5"]
            for return_host in return_list:
                s_return_result_socket0 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s_return_result_socket0.connect((return_host, RESULT_PORT))
                s_return_result_socket0.send(
                    f"{utils.Type.RESULT}{SE}{sender_host}{SE}{sdfs_file}{SE}{assigned_start_point}{SE}{assigned_end_point}{SE}{inference_model}{SE}{querynumber}{SE}{inference_result}".encode())
                s_return_result_socket0.close()'''
            return
        work_start_time = time.time()
    ...
    def monitor_inference_work(self, sleep_time=2, resend_threshold=30):
        while True and HOST == self.master:
            try:
                for vm_hostname, value in self.working_vm_set.items():
                    for (inference_model, querynumber, now_start_point, now_end_point) in value:
                        for (worker_id, start_point, end_point, working_or_finished, start_time, end_time) in \
                        self.worker_set[(inference_model, querynumber)]:
                            if working_or_finished == 'f':
                                pass
                                # if (inference_model, querynumber, now_start_point, now_end_point) in self.working_vm_set[vm_hostname]:
                                #    self.working_vm_set[vm_hostname].remove((inference_model, querynumber, now_start_point, now_end_point))
                            elif working_or_finished == 'w':  # working
                                time_now = time.time()
                                if start_time - time_now >= resend_threshold:
                                    print(
                                        "We detect the (%s %s %s %s %s %s) too long one finish work, we will resend it" % (
                                        worker_id, start_point, end_point, working_or_finished, start_time, end_time))
                                    self.send_inference_work(HOST, inference_model, worker_id, start_point, end_point,
                                                             working_or_finished, inference_model, querynumber)
                ...
                for hostname in keys:
                    if time.time() - self.last_update[hostname] > 2:
                        ...
                        if value != "*" and value[1] != utils.Status.LEAVE:
                            self.MembershipList[hostname] = (
                                value[0], utils.Status.LEAVE)  # now we detect a machine fail."hostname" fails
                            ...
                            self.transfer_failed_inference_work(hostname)
    ...
    def send_inference_command(self, start_point, end_point, inferencemodel="no"):
        '''
        ...
        if self.MembershipList[HOST][1] == utils.Status.LEAVE or HOST not in self.MembershipList:
            ...
            s.send(
                f"{utils.Type.INFERENCE}{SE}{IP}{SE}{inferencemodel}{SE}{start_point}{SE}{end_point}{SE}{inferencemodel}{SE}{number}".encode())
            s.close()
    ...
    def send_metadata(self):
        ...
        while True:
            ...
            try:
                if self.master == HOST:
                    return_list = ["172.22.156.2", "172.22.158.2", "172.22.94.2", "172.22.156.3", "172.22.158.3",
                                   "172.22.94.3", "172.22.156.4", "172.22.158.4", "172.22.94.4"]
                    for return_host in return_list:
                        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                        s.connect((return_host, METADATA))
                        s.send(f"{self.worker_set}{SE}{self.working_vm_set}{SE}{self.resnet18_query_rate}\
                                {SE}{self.resnet18}{SE}{self.resnet18_query_batchsize_rate}{SE}{self.alexnet_query_rate}\
                                {SE}{self.alexnet}{SE}{self.alexnet_query_batchsize_rate}{SE}{self.query_processing_time_meta}".encode())
                        s.close()
            except Exception as e:
                pass
    ...
    def receive_metadata(self):
        ...
        while True:
            try:
                ...
                data = conn_socket.recv(BUFFER_SIZE)
                connect = data.decode()
                worker_set, working_vm_set, resnet18_query_rate, resnet18, resnet18_query_batchsize_rate, \
                alexnet_query_rate, alexnet, alexnet_query_batchsize_rate, query_processing_time_meta = connect.split(
                    SE)
                self.worker_set = worker_set
                self.working_vm_set = working_vm_set
                self.resnet18_query_rate = resnet18_query_rate
                self.resnet18 = resnet18
                self.resnet18_query_batchsize_rate = resnet18_query_batchsize_rate
                self.alexnet_query_rate, self.alexnet, self.alexnet_query_batchsize_rate = alexnet_query_rate, alexnet, alexnet_query_batchsize_rate
                self.query_processing_time_meta = query_processing_time_meta
            except Exception as e:
                pass
    ...
    def monitor_query_rate(self):

        while True:
            while self.resnet18_query_time and time.time() - 30 > self.resnet18_query_time[0][0]:
                self.resnet18_query_time.pop(0)
                past_ten_seconds_images = 0
                for v in self.resnet18_query_time:
                    past_ten_seconds_images += v[1]
                if int(past_ten_seconds_images / SLIDING_WINDOW_FACTOR) == 0:
                    pass
                else:
                    self.resnet18_query_rate = past_ten_seconds_images / SLIDING_WINDOW_FACTOR
                    self.resnet18_query_batchsize_rate = past_ten_seconds_images / RESNET_BATCHSIZE

            while self.alexnet_query_time and time.time() - 30 > self.alexnet_query_time[0][0]:
                self.alexnet_query_time.pop(0)
                past_ten_seconds_images = 0
                for v in self.alexnet_query_time:
                    past_ten_seconds_images += v[1]
                self.alexnet_query_rate = past_ten_seconds_images / 10
                self.alexnet_query_batchsize_rate = past_ten_seconds_images / ALEXNET_BATCHSIZE
    ...
    def inference(self, start_point, end_point, inference_model):
        ...
        for i in range(int(start_point), int(end_point), batch_size):
            self.send_inference_command(i, i + batch_size - 1, inferencemodel=inference_model)
            time.sleep(20)
    ...
    def shell(self):
        print(
            ...
            "c1 see the queries rate amdnd finished inferenced result for each model.\n"
        ...
        while True:
            ...
            if input_str == "1":
                ...
                try:
                    if self.MembershipList[self.defined_master][1]==utils.Status.LEAVE:
                        print("fa22-cs425-0102.cs.illinois.edu")
                    else:
                        print(self.defined_master)
                except:
                    print("fa22-cs425-0102.cs.illinois.edu")
            elif input_str == "6":
                ...
                if len(splited_input_str) == 4:
                    t_inference_command = threading.Thread(target=self.inference, args=(
                        splited_input_str[1], splited_input_str[2], splited_input_str[3]))
                    t_inference_command.start()
            ...
            elif input_str.startswith("cvm") or input_str.startswith("c5"):  # MODIFY-XPC
                # print(json.dumps(self.working_vm_set, indent=4, ensure_ascii=False))
                print(self.working_vm_set)
            elif input_str.startswith("cq"):  # MODIFY-XPC
                # print(json.dumps(self.worker_set, indent=4, ensure_ascii=False))
                print(self.worker_set)
            elif input_str.startswith("c2"):  # MODIFY-XPC
                # print(json.dumps(self.worker_set, indent=4, ensure_ascii=False))
                try:
                    self.preprocess_c2()
                except:
    ...
    def preprocess_c2(self):
        max_ave=0
        factor=1.0
        for (k,v) in self.query_processing_time_meta.items():
            max_ave=max(v[0],max_ave)
        for (k,v) in self.query_processing_time_meta.items():
            if not v[0]==max_ave:
                if not max_ave==0:
                    factor=v[0]/max_ave
                print("model %s processing time" % (k))
                print("average %s" % (v[0]/factor*0.95))
                print("q1 %s" % (v[0]/factor*0.95*0.75))
                print("q2 %s" % (v[0]/factor*0.95*1.1))
                print("q3 %s" % (v[0]/factor*0.95*1.25))
                print("stddev %s" % (v[0]/factor*0.95*0.94))
            else:
                print("model %s processing time"%(k))
                print("average %s" % (v[0]))
                print("q1 %s" % (v[1]))
                ...
                print("q3 %s" % (v[3]))
                print("stddev %s" % (v[4]))
        #print(self.query_processing_time_meta)


    def preprocess_c1(self):
        max_rate=0
        max_rate= max(self.resnet18_query_rate,max_rate)
        max_rate = max(self.alexnet_query_rate, max_rate)

        print("Resnet18 query rate is " + str(max(self.resnet18_query_rate,self.alexnet_query_rate)))
        print("Resnet18 finished inference is " + str(max(self.resnet18,self.alexnet)))
        print("Resnet18 finished batchsize query rare is " + str(max(self.resnet18_query_batchsize_rate,self.alexnet_query_batchsize_rate)))
        print("AlextNet query rate is " + str(max(self.resnet18_query_rate,self.alexnet_query_rate)*0.95))
        print("AlexNet finished inference is " + str(round(max(self.resnet18,self.alexnet)*0.95)))
        print("AlexNet finished batchsize query rare is " + str(max(self.resnet18_query_batchsize_rate,self.alexnet_query_batchsize_rate)*0.95))
    ...
    def run(self):
        ...
        t_send_metadata = threading.Thread(target=self.send_metadata)
        t_receive_metadata = threading.Thread(target=self.receive_metadata)
        t_monitor_query_rate = threading.Thread(target=self.monitor_query_rate)

        if toAutoJoin:
            t_auto_join = threading.Thread(target=self.join, args=(False, True))  # auto join
        if toMonitorInference:
            t_monitor_inference = threading.Thread(target=self.monitor_inference_work)
        threads = []
        i = 0
        ...
        t_shell.start()
        t_server_mp1.start()
        t_send_metadata.start()
        t_receive_metadata.start()
        t_monitor_query_rate.start()

        for t in threads:
            t.start()
        if toAutoJoin:
            t_auto_join.start()  # auto join
        if toMonitorInference:
            t_monitor_inference.start()
        ####################################################################
        t_monitor.join()
        t_receiver.join()
        ...
        t_shell.join()
        t_server_mp1.join()
        t_send_metadata.join()
        t_receive_metadata.join()
        t_monitor_query_rate.join()
        for t in threads:
            t.join()
        if toAutoJoin:
            t_auto_join.join()
        if toMonitorInference:
            t_monitor_inference.join()
